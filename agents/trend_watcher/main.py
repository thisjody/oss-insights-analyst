...
def main(request):
    ...
    try:
        ...
        prompt = get_secret("trend_watcher_prompt")
    except Exception as e:
    ...
    user_instruction = input_data.get(
        "instruction",
        "Identify an open-source project that is currently trending and explain why.",
    )
    ...
    payload = {
        "contents": [
            {"role": "user", "parts": [{"text": prompt}]},
            {
                ...
                "parts": [
                    {
                        "text": f"Today's date is {current_date}. Use RAG to analyze trend data.\n\n{user_instruction}"
                    }
                ],
            },
        ]
